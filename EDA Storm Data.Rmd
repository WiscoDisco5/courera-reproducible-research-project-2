...
Note the stemming algorithm does seem to make for some funky looking words. Notice the word "lightn" is likely the stemmed version of "lightning". 

Now that we have a simpler set of strings representing each event, let's try clustering the events into similar groups. While a more thorough analysis would attempt to use the full dataset to cluster the events, I am going to cluster the events based on summarised results to speed things up. Since the clusters are going to be based on summarised results, I am only going to use words that appear more than 50 times to ensure credibility.
...
## make into matrix and take (log(x+.01))
names <- tokens_summarised$event_words
tokens_summarised$event_words <- NULL

logger <- function(x) {log(x+.01)} ##adding 0.01 for the 0's
...
##plot
plot(as.dendrogram(tokens_cluster), hang = -1, horiz = TRUE)
